...
vrep.simxFinish(-1) # 關掉先前的連接
clientID=vrep.simxStart('127.0.0.1',19997,True,True,5000,5) # Connect to V-REP
if clientID!=-1:
    print ('Connected to remote API server (連接到遠程API服務器)')
    ...
    while time.time()-startTime < 5:
        ...
        if returncode_pos==vrep.simx_return_ok: # After initialization of streaming, it will take a few ms before the first value arrives, so check the return code
            waitforfinish = vrep.simxSetObjectPosition(clientID, Cub1_handle, -1, (0.02*step, 0.02*step, 0.05), vrep.simx_opmode_oneshot_wait)
            print(data_pos)
            time.sleep(0.25)
