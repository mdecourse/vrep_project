...
class ControlPanel(QDialog, Ui_Daci):
    ...
    def print_go(self):
        ...
        self.print_output.addItem(f'[INFO] Select object: {self.edit_name1.text()}')
    '''
        
    @pyqtSlot(name='on_click_x_up_clicked')
    def xaxis(self):
        no_, handle_name1 = vrep.simxGetObjectHandle(client_ID, self.edit_name1.text(), vrep.simx_opmode_oneshot_wait)
    ...
    def return_(self):
        dm = 0.001
        step = 0.005
        no_, handle_designation = vrep.simxGetObjectHandle(client_ID, self.edit_name1.text(), vrep.simx_opmode_oneshot_wait)
    ...
    def with_go(self):
        ...
        def do(dx, dy, dz):
            ...
            handle, handle_value = vrep.simxGetObjectHandle(client_ID, self.edit_name1.text(), vrep.simx_opmode_oneshot_wait)
            ds = d // step
            # 移動量
            tx = dx / ds
            ty = dy / ds
    ...
    def __open_file__(self):
        ...
        if filename:
            ...
            for i in range(len(self.layer_vertices)):
                ...
                self.print_output.addItem(str(get))
