...
if clientID!=-1:
    ...
    for i in range(number):
        ...
        if returncode_pos==vrep.simx_return_ok: 
            waitforfinish = vrep.simxSetObjectPosition(clientID, Cub1_handle, -1, (0.0075*step, 0.015*number-0.01*step, 0.05), vrep.simx_opmode_oneshot_wait)
            print(data_pos)
    step = 0
    for i in range(number):
        step += 1
        returncode_pos, data_pos = vrep.simxGetObjectPosition(clientID, Cub1_handle, -1, vrep.simx_opmode_oneshot_wait)
        if returncode_pos==vrep.simx_return_ok: 
            waitforfinish = vrep.simxSetObjectPosition(clientID, Cub1_handle, -1, (0.0075*number+0.0075*step, 0.005*number+0.01*step, 0.05), vrep.simx_opmode_oneshot_wait)
            print(data_pos)
    for i in range(number):
        step -= 1
        returncode_pos, data_pos = vrep.simxGetObjectPosition(clientID, Cub1_handle, -1, vrep.simx_opmode_oneshot_wait)
